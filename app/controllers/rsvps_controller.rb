class RsvpsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @rsvp.save
        format.html { redirect_to @rsvp, notice: 'Bedankt voor jullie RSVP' }
  ...
  def update
    respond_to do |format|
      if @rsvp.update(rsvp_params)
        format.html { redirect_to @rsvp, notice: 'Je RSVP informatie is successvol gewijzigd' }
  ...
  private
    ...
    def rsvp_params
      params.require(:rsvp).permit(:name1, :name2, :ceremony, :reception, :diner)
    end
end
